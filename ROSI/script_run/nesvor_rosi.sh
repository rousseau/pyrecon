...
for file in ${images_list}
	...
	fonction(){
			...
			dir_output='/usr/local/NeSVoR/nesvor_rosi/'${sub_file}
			docker exec nesvor_contener mkdir '/usr/local/NeSVoR/nesvor_rosi/'
			docker exec nesvor_contener mkdir '/usr/local/NeSVoR/nesvor_rosi/'${sub_file}
			echo 'youpi'
		    docker exec nesvor_contener nesvor reconstruct --input-slices $dir_input  --output-volume $dir_output'/volume.nii' --output-slices $dir_output  --registration none --output-resolution 0.5 --inference-batch-size 255 --n-inference-samples 128  --output-slices $dir_output 
...
task (){
		...
		then
			...
			dir_input="/usr/local/NeSVoR/data/svort/"${suffix}"/svort_mvt"
			dir_output='/usr/local/NeSVoR/nesvor_rosi/'${suffix}
			echo $dir_input
			echo $dir_output
			docker exec nesvor_contener mkdir '/usr/local/NeSVoR/nesvor_rosi/'
			docker exec nesvor_contener mkdir '/usr/local/NeSVoR/nesvor_rosi/'${suffix}
		    docker exec nesvor_contener nesvor reconstruct --input-slices $dir_input  --output-volume $dir_output"/volume.nii" --output-slices $dir_output --registration none --output-resolution 0.5 --inference-batch-size 255 --n-inference-samples 128 

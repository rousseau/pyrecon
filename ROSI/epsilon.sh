...
do		
	...
	results='../res/epsilon/value'${epsilon}
	mkdir $results
	for file in ${images_list}
		...
		fonction(){
		     ...
		     output_registration=${results}'/'${sub_file}'/res_test_epsilon'${epsilon}
		      
		     
		     #echo "vive les herissons"
		     #
			 #python ROSI/main_realdata.py --filenames $list  --filenames_masks $list_mask  --output $output_registration  --ablation no_multistart no_dice --hyperparameters 4 0.3 2000 $epsilon 1 0 
		     #echo "les herissons n ont pas de cerveau"

		  
		     output_reconstruction_inter=${output_path}'/'${sub_file}'/intersection_test.nii.gz'
		     output_reconstruction_pipeline='NiftyMIC/ipta/'${sub_file}'/epsilon'${epsilon}
		    
		     dir_output=${output_path}'/'${sub_file}'/pipeline_epsilon'${epsilon}'/recon_subject_space/motion_correction'
             ...
             docker exec wizardly_brattain mkdir 'NiftyMIC/ipta/'${file1}'/'${file2}'/epsilon'${epsilon}

		     docker cp ${output_registration}'_mvt' wizardly_brattain:'/app/NiftyMIC/ipta/'${file1}'/'${file2}'/epsilon'${epsilon}'/res_test_epsilon'${epsilon}'_mvt'

		     dir_output_motion='/app/NiftyMIC/ipta/'${sub_file}'/epsilon'${epsilon}'/res_test_epsilon'${epsilon}'_mvt'
		     ...
		     docker exec wizardly_brattain python NiftyMIC/niftymic_run_reconstruction_pipeline_slices.py --filenames "${list_docker[@]}" --filenames-masks "${mask_docker[@]}" --dir-output $output_reconstruction_pipeline  --dir-input-mc $dir_output_motion 
	     	 docker cp wizardly_brattain:'/app/NiftyMIC/ipta/'${sub_file}'/epsilon'${epsilon} ${results}'/'${sub_file}'/'
	...
	task (){
		...
		then
			...
			mkdir ${results}'/simul_data/'${suffix}'/epsilon'${epsilon}
			...
			output_simul_path=${results}'/simul_data/'${suffix}/'epsilon'${epsilon}
			output_simul=${output_simul_path}'/res_test_epsilon'${epsilon}
			mkdir output_simul
			output_res='NiftyMIC/ipta/'${suffix}'/epsilon'${epsilon}
			...
			#python ROSI/main.py --filenames ${image}  --filenames_masks "${mask[@]}" --nomvt "${nomvt[@]}" --simulation "${transfo[@]}" --output ${output_simul}  --ablation no_multistart no_dice --hyperparameters 4 0.3 2000 $epsilon 1 0 
			docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix} 
			docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix}'/epsilon'${epsilon}
			docker cp ${output_simul}'_mvt'  e91b0f478887:'/app/NiftyMIC/ipta/'${suffix}'/epsilon'${epsilon}'/res_test_epsilon'${epsilon}'_mvt' 
			dir_output_motion='/app/NiftyMIC/ipta/'${suffix}'/epsilon'${epsilon}'/res_test_epsilon'${epsilon}'_mvt'
			...
			docker cp e91b0f478887:'/app/NiftyMIC/ipta/'${suffix}'/epsilon'${epsilon} ${results}'/simul_data/'${suffix}'/epsilon'${epsilon}

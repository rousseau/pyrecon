...
def createVolumesFromAlist(listSlice):
    ...
    for s in listSlice:
        s_or = s.get_index_image()#s.get_index_image()
...
def distance_from_mask(volume):

    inv_chamfer_distance = distance_transform_cdt(1-volume)

    return inv_chamfer_distance

def computeMaxVolume(listSlice,nbstack,res):
    V =  np.sum([np.sum(slicei.get_mask()) for slicei in listSlice])
    print(nbstack)
    Vmx=V

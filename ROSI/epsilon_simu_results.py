...
value_epsilon=["0.5","1","2","4"]


for test_epsilon in value_epsilon:
    errorlist=[]
    psnrlist=[]
    ssimlist=[] 
    for index_image in range(1,6):
        #error_before=[]
        file = '../../res/epsilon/value%s/simul_data/tres_petit%d/epsilon%s/epsilon%s' %(test_epsilon,index_image,test_epsilon,test_epsilon)
        petit = '../../res/epsilon/value%s/simul_data/tres_petit%d/epsilon%s/res_test_epsilon%s.joblib.gz' %(test_epsilon,index_image,test_epsilon,test_epsilon)
...
for test_epsilon in value_epsilon:   
    ...
    for index_image in range(1,6):
        ...
        file='../../res/epsilon/value%s/simul_data/Petit%d/epsilon%s/epsilon%s' %(test_epsilon,index_image,test_epsilon,test_epsilon)
        petit = '../../res/epsilon/value%s/simul_data/Petit%d/epsilon%s/res_test_epsilon%s.joblib.gz' %(test_epsilon,index_image,test_epsilon,test_epsilon)
...
for test_epsilon in value_epsilon:
    ...
    for index_image in range(1,6):
        ...
        file = '../../res/epsilon/value%s/simul_data/Moyen%d/epsilon%s/epsilon%s' %(test_epsilon,index_image,test_epsilon,test_epsilon)
        petit = '../../res/epsilon/value%s/simul_data/Moyen%d/epsilon%s/res_test_epsilon%s.joblib.gz' %(test_epsilon,index_image,test_epsilon,test_epsilon)
        #directory1 = '/home/mercier/Documents/donnee/test/Grand%d/' %(i)
...
for test_epsilon in value_epsilon:
    ...
    for index_image in range(1,6):
        ...
        petit = '../../res/epsilon/value%s/simul_data/Grand%d/epsilon%s/res_test_epsilon%s.joblib.gz' %(test_epsilon,index_image,test_epsilon,test_epsilon)
        #directory1 = '/home/mercier/Documents/donnee/test/Grand%d/' %(i)
...
for test_epsilon in range(len(value_epsilon)):
        print('test :',test_epsilon)
        ...
        for mvt in range(0,4):
            ...
            data=datalist[test_epsilon]
        
                #print(len(before))
                #print(len(data))
                
                #add error from each epsilon test
            ...
            if len(data)==len(nous_before):
                ...
                axs[test_epsilon].scatter(data,nous_before,marker='.',s=170,alpha=0.1,c=color[mvt])
                axs[test_epsilon].set_ylabel('avant recalage',fontsize=15)
                axs[test_epsilon].set_xlabel('après recalage',fontsize=15)
                axs[test_epsilon].set_title(('epsilon=%s' %(value_epsilon[test_epsilon])),fontsize=15) 
                axs[test_epsilon].set_ylim(0,16)
                axs[test_epsilon].set_xlim(0,5)

        for tick in axs[test_epsilon].xaxis.get_majorticklabels():  # example for xaxis
                tick.set_fontsize(15) 

        for tick in axs[test_epsilon].yaxis.get_majorticklabels():  # example for xaxis
                tick.set_fontsize(15) 
...
plt.savefig('epsilon_test.png')

for i in range(0,len(value_epsilon)):
    
    print('epsilon :',value_epsilon[i])
    print('PSNR')

...
import numpy as np
import random as rd
from registration import commonSegment, commonSegment2
...
def findCommonPointbtw2V(Volume1,Volume2,transfo1,transfo2,rejectedSlices):
    
    listPointVolume1 = []
    listPointVolume2 = []
    #print('taille volume :',len(Volume1))
    for zV1 in range(len(Volume1)):
        ...
        for zV2 in range(len(Volume2)):
            ...
            if ((sV1.get_orientation(),sV1.get_index_slice()) not in rejectedSlices) and ((sV2.get_orientation(),sV2.get_index_slice()) not in rejectedSlices) :
                ...
                #print('indexlist :',Volume1[zV1].get_index_slice())
                sliceimage1=sV1.get_slice().get_fdata();M1=transfo1[Volume1[zV1].get_index_slice(),:,:] @ sV1.get_slice().affine #@ sV1.get_transfo()
                sliceimage2=sV2.get_slice().get_fdata();M2=transfo2[Volume2[zV2].get_index_slice(),:,:] @ sV2.get_slice().affine #@ sV2.get_transfo()
                res=min(min(sV1.get_slice().header.get_zooms(),sV2.get_slice().header.get_zooms()))
                pt1,pt2,nbpoint,ok = commonSegment2(sliceimage1,M1,sliceimage2,M2,res)
                #print(nbpoint[0,0],np.sum(index1!=index2))
                #pt1,pt2,nbpoint,ok = commonSegment2(sliceimage1,M1,sliceimage2,M2,res)
                ...
                if ok[0,0]>0:
                    ...
                    ptInterpol1 = np.zeros((3,nbpt))
                    ptInterpol1[0,:] = pointV1[:,0]
                    ptInterpol1[1,:] = pointV1[:,1]
                    ptInterpol1[2,:] = np.zeros(nbpt)
                    ...
                    ptInterpol2 = np.zeros((3,nbpt))
                    ptInterpol2[0,:] = pointV2[:,0]
                    ptInterpol2[1,:] = pointV2[:,1]
                    ptInterpol2[2,:] = np.zeros(nbpt)
                    
                    
                    map_coordinates(maskV1, ptInterpol1, output=interpolMaskV1, order=0, mode='constant',cval=np.nan,prefilter=False)
                    map_coordinates(maskV2, ptInterpol2, output=interpolMaskV2, order=0, mode='constant',cval=np.nan,prefilter=False)
                    
                    #print(interpolMaskV1.shape,nbpt)
                    
                    common_mask = [interpolMaskV1[index] or interpolMaskV2[index] for index in range(0,nbpt)]
                    #print(np.where(interpolMaskV1!=interpolMaskV2))
                    #print(interpolMaskV1[(interpolMaskV1>0) * (interpolMaskV1<1)])
                    common_mask=np.array(common_mask,dtype=bool)
                    #print(len(common_mask>0))
    ...
    for indice  in range(len(listPointVolume1)):
            ...
            transV1 = np.linalg.inv(transfo1[Volume1[zV1].get_index_slice(),:,:]) @ Volume1[zV1].get_transfo()
            transV2 = np.linalg.inv(transfo2[Volume2[zV2].get_index_slice(),:,:]) @ Volume2[zV2].get_transfo() 
            pwv1 = np.zeros(4)
            pwv1[0:2] = p1[0:2]
            pwv1[3] = 1
            pwv2 = np.zeros(4)
...
def error_for_each_slices(listnomvt,listSlice,listSliceError,transfo,rejected_slice):
    ...
    for i1 in range(len(images)):
        for i2 in range(len(images)):
            if i1 < i2:
               ...
               transfo1 = np.load(transfo[i1])
               transfo2 = np.load(transfo[i2])
               ptimg1img2_img1, ptimg1img2_img2 = findCommonPointbtw2V(images[i1],images[i2],transfo1,transfo2,rejected_slice) #common points between volumes when no movement
               listptimg1img2_img1.append(ptimg1img2_img1)
               listptimg1img2_img2.append(ptimg1img2_img2)
    ...
    for i1 in range(len(images)):
        for i2 in range(len(images)):
            if i1 < i2:
                   ...
                   transfo1 = np.load(transfo[i1])
                   transfo2 = np.load(transfo[i2])
                   errorimg1img2_after = ErrorOfRegistrationBtw2SliceBySlice(listptimg1img2_img1[indice],listptimg1img2_img2[indice],images_corrected[i1],images_corrected[i2],np.linalg.inv(transfo1),np.linalg.inv(transfo2),listvolumeSliceError[i1],listvolumeSliceError[i2])
                   listerrorimg1img2_after.append(errorimg1img2_after)
                   indice=indice+1

...
#2. Correction of bias fields
#3. Reconstruction of an HR image in subject-space using the input transformation.
#4. Registration of the resulting volume in template space
...
def main():
    ...
    intersection = os.path.join(dir_output_recon_subject_space,"%s" %"intersection.nii.gz")
    ...
    cmd_args = ["python main_realdata.py"]
    cmd_args.append("--filenames %s" % (" ").join(filenames))
    cmd_args.append("--filenames_masks %s" %(" ").join(filenames_masks))
    cmd_args.append("--output %s" % intersection)               
    cmd_args.append("--hyperparameters 4 0.25 1e-10 2 10 4")
    cmd_args.append("--ablation all")

    #cmd = (" ").join(cmd_args)
    #exit_code = ph.execute_command(cmd)
    #if exit_code != 0:
    #    raise RuntimeError("Motion correction in subject space failed")
        #elapsed_time_recon_subject_space = ph.stop_timing(time_start)

    dir_motion=args.dir_input_mc#"%s_mvt" %intersection
    new_dir=os.path.join(dir_output_recon_subject_space,"motion_correction")
    #print(os.getcwd())
    os.mkdir(dir_output_recon_subject_space)
    print('Output created')
    #if os.path.exists(new_dir):
    #    shutil.rmtree(new_dir)
    shutil.copytree(dir_motion,os.path.join(dir_output_recon_subject_space,"motion_correction"))
    dir_motion_correction=os.path.join(dir_output_recon_subject_space,"motion_correction")
    ...
    if args.run_recon_subject_space:
        ...
        if 0:
            ...
            #dir_motion_correction=dir_input_mc
            cmd_args = ["niftymic_reconstruct_volume_from_slices"]
            cmd_args.append("--filenames %s" % " ".join(filenames_masks))
            cmd_args.append("--dir-input-mc '%s'" % dir_motion_correction)
            cmd_args.append("--output '%s'" % srr_subject_mask)
    ...
    snap_ax=os.path.join(
        dir_output_recon_template_space,
        "%ssnap_axial.png" % args.prefix_output)
    snap_cor=os.path.join(
        dir_output_recon_template_space,
        "%ssnap_cor.png" % args.prefix_output)
    snap_sag=os.path.join(
        dir_output_recon_template_space,
        "%ssnap_sagital.png" % args.prefix_output)

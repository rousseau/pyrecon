...
echo $images_file
images_list="$(find $images_file -name '*sub-0034*'  -type d)"
...
for file in ${images_list}
	...
	fonction(){
		     ...
		     output_registration=${results}'/'${sub_file}'/res_test_all'
		      
		     
		     #echo "vive les herissons"
		     #
			 #python ROSI/main_realdata.py --filenames $list  --filenames_masks $list_mask  --output $output_registration  --ablation multistart no_dice Nelder-Mead --hyperparameters 4 0.25 2000 2 1 0 
		     #echo "les herissons n ont pas de cerveau"
			 nesvor reconstruct --input-stacks $list --stack-masks  $list_mask  --output-volume $output_registration  
		  
		     output_reconstruction_inter=${output_path}'/'${sub_file}'/intersection_test.nii.gz'
		     output_reconstruction_pipeline='NiftyMIC/ipta/'${sub_file}'/all'
		    
		     dir_output=${output_path}'/'${sub_file}'/pipeline_all/recon_subject_space/motion_correction'
             ...
             #docker exec wizardly_brattain mkdir 'NiftyMIC/ipta/'${file1}'/'${file2}'/all'${all}
...
task (){
		...
		then
			...
			output_res='NiftyMIC/ipta/'${suffix}'/all'
					...
					#echo ${output_res}
		    echo "${mask[@]}"
			python ROSI/main.py --filenames ${image}  --filenames_masks "${mask[@]}" --nomvt "${nomvt[@]}" --simulation "${transfo[@]}" --output ${output_simul}  --ablation no_multistart no_dice --hyperparameters 4 0.3 2000 2 $all 0 
			docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix} 
			docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix}'/all'
			docker cp ${output_simul}'_mvt'  e91b0f478887:'app/NiftyMIC/ipta/'${suffix}'/all/res_test_all_mvt' 
			dir_output_motion='NiftyMIC/ipta/'${suffix}'/all/res_test_all_mvt'
			...
			docker cp e91b0f478887:'/app/NiftyMIC/ipta/'${suffix}'/all' ${results}'/simul_data/'${suffix}'/all'

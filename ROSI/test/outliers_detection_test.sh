...
output_path='../res/outliers_detection'
mkdir $output_path
echo $images_list
...
do		
	...
	results='../res/outliers_detection/value'${omega}
	mkdir $results
	for file in ${images_list}
		...
		fonction(){
		     ...
		     python main_realdata.py --filenames $list  --filenames_masks $list_mask  --output $output_registration  --ablation multistart dice --hyperparameters 4 0.25 2000 2 $omega 0 
	...
	#	echo $nomvt
		...
		python main_newsimul.py --filenames ${image}  --filenames_masks "${mask[@]}" --nomvt "${nomvt[@]}" --simulation "${transfo[@]}" --output ${output_simul}  --ablation multistart dice --hyperparameters 4 0.25 2000 2 $omega 0 

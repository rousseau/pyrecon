...
do		
	...
	for file in ${images_list}
		...
		fonction(){
		     ...
		     python main_realdata.py --filenames $list  --filenames_masks $list_mask  --output $output_registration  --ablation no_multistart no_dice --hyperparameters $simplex 0.25 2000 2 1 0 
	...
	task (){
		...
		then
			...
			python main_newsimul.py --filenames ${image}  --filenames_masks "${mask[@]}" --nomvt "${nomvt[@]}" --simulation "${transfo[@]}" --output ${output_simul}  --ablation no_multistart no_dice --hyperparameters $simplex 0.25 2000 2 1 0 

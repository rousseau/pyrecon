...
output_path='../res/gaussian_test_v2'
mkdir $output_path
echo $images_list
...
do		
	...
	results='../res/gaussian_test_v2/value'${omega}
	mkdir $results
	for file in ${images_list}
		...
		fonction(){
		     ...
		     python main_realdata.py --filenames $list  --filenames_masks $list_mask  --output $output_registration  --ablation no_multistart dice --hyperparameters 4 0.25 2000 2 $omega 0 
	...
	#	echo $nomvt
		...
		python main_newsimul.py --filenames ${image}  --filenames_masks "${mask[@]}" --nomvt "${nomvt[@]}" --simulation "${transfo[@]}" --output ${output_simul}  --ablation no_multistart dice --hyperparameters 4 0.25 2000 2 $omega 0 
	...
	docker exec e91b0f478887 rm -r '/app/NiftyMIC/ipta'
	docker exec e91b0f478887 mkdir '/app/NiftyMIC/ipta'

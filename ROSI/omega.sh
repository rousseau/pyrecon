...
for file in ${images_list}
	...
	fonction(){
		     ...
		     #
			 python ROSI/main_realdata.py --filenames $list  --filenames_masks $list_mask  --output $output_registration  --ablation no_multistart no_dice --hyperparameters 4 0.3 2000 2 $omega 0 
		     ...
		     dir_output_motion='NiftyMIC/ipta/'${sub_file}'/omega'${omega}'/res_test_omega'${omega}'_mvt'
...
task (){
		...
		then
			...
			mkdir ${results}'/simul_data/'${suffix}'/omega'${omega}
			...
			output_simul_path=${results}'/simul_data/'${suffix}/'omega'${omega}
			output_simul=${output_simul_path}'/res_test_omega'${omega}
			mkdir output_simul
			output_res='NiftyMIC/ipta/'${suffix}'/omega'${omega}
					...
					#echo ${output_res}
		    echo "${mask[@]}"
			python ROSI/main.py --filenames ${image}  --filenames_masks "${mask[@]}" --nomvt "${nomvt[@]}" --simulation "${transfo[@]}" --output ${output_simul}  --ablation no_multistart no_dice --hyperparameters 4 0.3 2000 2 $omega 0 
			docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix} 
			docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix}'/omega'${omega}
			docker cp ${output_simul}'_mvt'  e91b0f478887:'app/NiftyMIC/ipta/'${suffix}'/omega'${omega}'/res_test_omega'${omega}'_mvt' 
			dir_output_motion='NiftyMIC/ipta/'${suffix}'/omega'${omega}'/res_test_omega'${omega}'_mvt'
			...
			docker cp e91b0f478887:'/app/NiftyMIC/ipta/'${suffix}'/omega'${omega} ${results}'/simul_data/'${suffix}'/omega'${omega}

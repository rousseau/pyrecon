...
def main():
    ...
    if args.tre==1:
        ...
        res_obj = [('listSlice',listSlice),('ErrorEvolution',ErrorEvolution), ('DiceEvolution',DiceEvolution), ('EvolutionGridError',EvolutionGridError), ('EvolutionGridNbpoint',EvolutionGridNbpoint), ('EvolutionGridInter',EvolutionGridInter), ('EvolutionGridUnion',EvolutionGridUnion), ('RejectedSlices',rejectedSlices)]
    ...
    container = client.containers.get("ebner")

    subprocess.run(["python" ,"main.py"],input=)

    container.exec_run('mkdir NiftyMIC/test')
    container.exec_run('mkdir NiftyMIC/test/%s/' %(sub))
    container.exec_run('mkdir NiftyMIC/test/%s/%s/' %(sub,ses))
    container.exec_run('mkdir python NiftyMIC/niftymic_run_reconstruction_pipeline_slices.py ') 

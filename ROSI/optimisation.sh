...
for optimisation in "Nelder-Mead"
do
	output_path='../res/'${optimisation}
	mkdir $output_path
	results='../res/'${optimisation}'/value'
	mkdir $results
	echo "opti"
	echo $optimisation
	...
	for file in ${images_list}
			...
			fonction(){
				...
				output_registration=${results}'/'${sub_file}'/res_test_'${optimisation}
				
				
				#echo "vive les herissons"
				#
				#python ROSI/main_realdata.py --filenames $list  --filenames_masks $list_mask  --output $output_registration  --ablation no_multistart no_dice --hyperparameters 4 0.3 2000 0.25 0 0 
				#echo "les herissons n ont pas de cerveau"

			
				output_reconstruction_inter=${output_path}'/'${sub_file}'/intersection_test.nii.gz'
				output_reconstruction_pipeline='/app/NiftyMIC/ipta/'${sub_file}'/'${optimisation}
				
				dir_output=${output_path}'/'${sub_file}'/pipeline_'${optimisation}'/recon_subject_space/motion_correction'
				...
				docker exec wizardly_brattain mkdir '/app/NiftyMIC/ipta/'${file1}'/'${file2}'/'${optimisation}

				docker cp ${output_registration}'_mvt' wizardly_brattain:'/app/NiftyMIC/ipta/'${file1}'/'${file2}'/'${optimisation}'/res_test_'${optimisation}'_mvt'

				dir_output_motion='/app/NiftyMIC/ipta/'${sub_file}'/'${optimisation}'/res_test_'${optimisation}'_mvt'
				...
				docker cp wizardly_brattain:'/app/NiftyMIC/ipta/'${sub_file}'/'${optimisation}  ${results}'/'${sub_file}'/'
	...
	task (){
			...
			then				
				...
				mkdir ${results}'/simul_data/'${suffix}'/'${optimisation}
				...
				output_simul_path=${results}'/simul_data/'${suffix}'/'${optimisation}
				output_simul=${output_simul_path}'/res_test_'${optimisation}
				mkdir output_simul
				output_res='NiftyMIC/ipta/'${suffix}'/'${optimisation}
				...
				python ROSI/main.py --filenames ${image}  --filenames_masks "${mask[@]}" --nomvt "${nomvt[@]}" --simulation "${transfo[@]}" --output ${output_simul}  --ablation no_multistart no_dice $optimisation --hyperparameters 4 0.3 2000 0.25 0 0 
				#docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix} 
				#docker exec e91b0f478887 mkdir 'NiftyMIC/ipta/'${suffix}'/${optimisation}'${${optimisation}}
				#docker cp ${output_simul}'_mvt'  e91b0f478887:'/app/NiftyMIC/ipta/'${suffix}'/${optimisation}'${${optimisation}}'/res_test_${optimisation}'${${optimisation}}'_mvt' 
				#dir_output_motion='/app/NiftyMIC/ipta/'${suffix}'/${optimisation}'${${optimisation}}'/res_test_${optimisation}'${${optimisation}}'_mvt'
				...
				#docker cp e91b0f478887:'/app/NiftyMIC/ipta/'${suffix}'/${optimisation}'${${optimisation}} ${results}'/simul_data/'${suffix}'/${optimisation}'${${optimisation}}
